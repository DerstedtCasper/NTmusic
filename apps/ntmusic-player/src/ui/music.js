...
class WebNowPlayingAdapter {
   ...
   const populateDeviceList = async () => {
       ...
       if (result.status === 'success' && result.devices) {
           ...
           // Add ASIO devices
           if (result.devices.asio && result.devices.asio.length > 0) {
               const asioGroup = document.createElement('optgroup');
               asioGroup.label = 'ASIO';
               result.devices.asio.forEach(device => {
                   const option = document.createElement('option');
                   option.value = device.id;
                   option.textContent = device.name;
                   asioGroup.appendChild(option);
               });
               deviceSelect.appendChild(asioGroup);
           }
           // Add other devices
           if (result.devices.other && result.devices.other.length > 0) {
               const otherGroup = document.createElement('optgroup');
               otherGroup.label = 'Other';
               result.devices.other.forEach(device => {
                   const option = document.createElement('option');
                   option.value = device.id;
                   option.textContent = device.name;
