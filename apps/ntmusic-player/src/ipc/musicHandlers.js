...
async function scanLibraryFromPath(event, folderPath) {
    if (!folderPath) {
        return { status: 'error', message: 'scan path missing', tracks: [] };
    }

    event.sender.send('scan-started', { total: 0 });
    event.sender.send('library:scan-started', { total: 0, path: folderPath });

    const result = await audioEngineApi('/library/scan', 'POST', { path: folderPath });
    if (!result || result.status !== 'success') {
        const message = result?.message || 'library scan failed';
        event.sender.send('scan-finished', []);
        event.sender.send('library:result', { status: 'error', message, tracks: [] });
        return { status: 'error', message, tracks: [] };
    }

    const tracks = Array.isArray(result.tracks) ? result.tracks : [];
    const total = tracks.length;
    event.sender.send('scan-started', { total });
    event.sender.send('library:scan-started', { total, path: folderPath });

    let done = 0;
    for (const track of tracks) {
        done += 1;
        event.sender.send('scan-progress');
        event.sender.send('library:progress', { done, total, track });
    }

    event.sender.send('scan-finished', tracks);
    event.sender.send('library:result', { status: 'success', tracks, total });
    return { status: 'success', tracks, total };
}
...
function initialize(options) {
    ...
    if (options.openChildWindows) {
        openChildWindows = options.openChildWindows;
    }
    if (options.startAudioEngine) {
        startAudioEngine = options.startAudioEngine;
    }
    ...
    const registerIpcHandlers = () => {
        ...
        ipcMain.handle('library:scan', async (event, { path: requestedPath } = {}) => {
            if (typeof startAudioEngine === 'function') {
                try {
                    await startAudioEngine();
                } catch (error) {
                    const message = error?.message || 'engine start failed';
                    event.sender.send('library:result', { status: 'error', message, tracks: [] });
                    return { status: 'error', message, tracks: [] };
                }
            }
            ...
            if (!folderPath) {
                ...
                if (result.canceled || result.filePaths.length === 0) {
                    return { status: 'cancelled', tracks: [] };
                }
                folderPath = result.filePaths[0];
            }

            return scanLibraryFromPath(event, folderPath);
        });
        ...
        ipcMain.on('open-music-folder', async (event) => {
            ...
            try {
                await scanLibraryFromPath(event, folderPath);
            } catch (err) {
                console.error("Error during music scan:", err);
                event.sender.send('scan-finished', []);
            }
        });
...
module.exports = {
    ...
    openMusicWindow: () => createOrFocusMusicWindow()
};
