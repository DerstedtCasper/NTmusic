...
contextBridge.exposeInMainWorld('player', {
    load: (path) => ipcRenderer.invoke('player:load', { path }),
    play: (path) => ipcRenderer.invoke('player:play', path ? { path } : {}),
    resume: () => ipcRenderer.invoke('player:resume'),
    pause: () => ipcRenderer.invoke('player:pause'),
    stop: () => ipcRenderer.invoke('player:stop'),
    selectDevice: (deviceId, exclusive) =>
        ipcRenderer.invoke('player:set-device', { deviceId, exclusive }),
    getDevices: () => ipcRenderer.invoke('player:get-devices'),
    getTrack: () => ipcRenderer.invoke('player:get-track'),
    getPosition: () => ipcRenderer.invoke('player:get-position'),
    queueAdd: (tracks, replace) => ipcRenderer.invoke('player:queueAdd', { tracks, replace }),
    next: () => ipcRenderer.invoke('player:next'),
    captureStart: (options = {}) => ipcRenderer.invoke('player:captureStart', options),
    captureStop: () => ipcRenderer.invoke('player:captureStop'),
    onProgress: (callback) => {
        const handler = (_event, payload) => {
            if (!payload || payload.type !== 'playback.state' || !payload.state) return;
            const current = Number(payload.state.current_time || 0);
            const duration = Number(payload.state.duration || 0);
    ...
    onTrackEnd: (callback) => {
        ...
        const handler = (_event, payload) => {
            if (!payload || payload.type !== 'playback.state' || !payload.state) return;
            const isPlaying = Boolean(payload.state.is_playing && !payload.state.is_paused);
            const duration = Number(payload.state.duration || 0);
            const current = Number(payload.state.current_time || 0);
            const filePath = payload.state.file_path || null;
            if (lastPlaying && !isPlaying && duration > 0 && current >= duration - 0.05) {
                callback({ path: filePath, duration });
            }
